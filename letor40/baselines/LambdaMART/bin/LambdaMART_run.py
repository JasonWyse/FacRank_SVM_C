...
def validation(**pars):
	pars_name=pars.iterkeys()
	pars_value=pars.itervalues()
...
# tune the parameters
	for fold_number in [1,2,3,4,5]:
		...
		Rate=0.00001
		round=200	
		for r in range(1,round+1,10):
			for leaf in range(2,20,2):
				for shrinkage in[0.001,0.005,0.01,0.05,0.1]:
			#for rate in [0.00001,0.0001,0.001,0.01]:
					print os.popen(command_train%(train,6,model,r,leaf,shrinkage))
		...
		F_pars.write('Fold%d\n%s\t%s\t%s\t%s\n'%(fold_number,BestRound,Rate,Leaf,Shrinkage))
...
command_train='java -jar '+tls+' -train %s -ranker %d -save %s -metric2t MAP -tree %d -leaf %d -shrinkage %f'
...
validation(round=[1,2,3],shrikage=[0.1,0.001],leaf=[2,4])
