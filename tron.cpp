...
void TRON::tron(double *w)
{
	...
	while (iter <= max_iter && search)
	{
#ifdef FIGURE56
#else
		info("FUN %20.15e TIME %20.15e\n", f,double(clock()-start)/double(CLOCKS_PER_SEC));
#endif
		...
		info("iter %2d act %5.3e pre %5.3e delta %5.3e f %20.16e |g| %5.3e CG %3d\n", iter, actred, prered, delta, f, gnorm, cg_iter);
#ifdef FIGURE56
		total += clock()-start;
	...
	if (iter > max_iter)
		info("WARNING: MAX ITERATION OF TRON REACHED\n");
...
void TRON::_tron(model* model_,const problem *prob)
{
	int round = model_->param.ite;
	double *w = model_->w;	
	...
	int *perm = Malloc(int,l);//Malloc(int,l);//perm: indices to the original data, ie.  perm[i]=i;(i=0,1,...,l)
	...
	double f_old = fun_obj->_fun(model_->w);
	double f;
	tstart = clock();	
	while(round>0)
	{
		fun_obj->_grad(model_,prob, nr_subset, perm, start, count);	//update model_->param.v_pp		
		v_pp_to_w(model_->w, model_->param.v_pp,&nr_subset,start,count,perm,prob->y,&model_->param,prob);
		f = fun_obj->_fun(model_->w);//actually we don't need to calculate f
		if(f<f_old)
			f_old = f;
		else
			break;
